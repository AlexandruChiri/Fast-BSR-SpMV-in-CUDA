...
__device__ __forceinline__ void one_row_of_blocks(
    ...
    T acc = 0;
    const int workers = blockDim.x - blockDim.x % RC;
    const int teams = blockDim.x / RC;
    const int team_id = threadIdx.x / RC;
    const I start = start_index / RC;
    const I stop = stop_index / RC;
    const int worker_id = threadIdx.x % RC;
    const I i_b = worker_id / C;
    const I j_b = worker_id % C;
    const I index_b = worker_id;

    if (threadIdx.x < workers) {
        for (I jj = start + team_id; jj < stop; jj += teams) {
            // The block column
            const I j = Aj[jj];
            // The column coordinate within the matrix
            const I j_final = C * j + j_b;
            const npy_intp index = jj * RC + index_b;
            // Atomicity required
            acc += Ax[index] * Xx[j_final];
        }
        atomicAdd(&sum_private[i_b], acc);
    }
